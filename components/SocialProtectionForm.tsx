...
const SocialProtectionForm: React.FC<SocialProtectionFormProps> = ({ 
  ...
  const handleAddToList = () => {
    ...
    if (editingTempIndex !== null) {
      ...
      newList[editingTempIndex] = { ...currentRecord };
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[95vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
          <div className="space-y-4">
            ...
            {selectedHouse ? (
              <div className={`border rounded-xl p-4 flex items-center justify-between shadow-sm ${isHouseLocked || isEditing ? 'bg-slate-50 border-slate-200' : 'bg-emerald-50 border-emerald-100'}`}>
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold ${isHouseLocked || isEditing ? 'bg-slate-200 text-slate-500' : 'bg-emerald-100 text-emerald-600'}`}>
                ...
                {!isEditing && !isHouseLocked && (
                  <button onClick={() => { setSelectedHouseId(undefined); setRecordsList([]); }} className="p-2 hover:bg-red-50 text-red-500 rounded-lg transition-colors">
                    <Trash2 size={18} />
                  </button>
                )}
              </div>
                          ...
                          recordsList.map((r, idx) => (
                            <tr key={idx} className={`hover:bg-slate-50 group transition-colors ${editingTempIndex === idx ? 'bg-orange-50/50' : ''}`}>
                              ...
                              <td className="px-4 py-3">
                                 {r.HinhThucNhan === 'Chuyển khoản' ? (
                                  <div className="flex flex-col">
                                    ...
                                    <span className="text-[9px] text-slate-400 font-mono">{r.SoTaiKhoan}</span>
                                  </div>
                                ) : (
                              ...
                              <td className="px-4 py-3 text-center">
                                {r.NguoiNhanThay ? <span className="text-[10px] text-emerald-600 font-bold bg-emerald-50 px-2 py-0.5 rounded">{r.NguoiNhanThay}</span> : <span className="text-[10px] text-slate-400">Chính chủ</span>}
                              </td>
        ...
        <div className="p-6 border-t bg-slate-50 flex justify-end gap-3 shrink-0">
          ...
          <button 
            ...
            disabled={!selectedHouseId || (!isEditing && recordsList.length === 0)}
            className={`px-8 py-2 rounded-xl shadow-lg flex items-center gap-2 transition-all transform active:scale-95 font-bold ${(!selectedHouseId || (!isEditing && recordsList.length === 0)) ? 'bg-slate-200 text-slate-400 cursor-not-allowed' : 'bg-emerald-600 hover:bg-emerald-700 text-white transform hover:scale-105'}`}
          >
